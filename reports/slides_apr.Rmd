...
```{r tabela fontes,  echo = FALSE, fig.height=3, fig.width=7}
# Tabela
...
```{r, echo = FALSE}
rec_tbl |> 
  kbl(booktabs = T, caption = "Harding Pagan vs CODACE", 
    format = "html") |> 
  kable_paper(full_width = F) |> 
    footnote("Elaboração Própria", footnote_as_chunk = T)
...
- colocar uma tabela com as váriaveis selecionadas.
...
tcb_tbl |> 
  kbl(booktabs = T, caption = "Indicadores Coincidentes de Atividade Econômica", 
    format = "html") |> 
  kable_paper(full_width = F) |> 
    footnote("Elaboração Própria", footnote_as_chunk = T)
